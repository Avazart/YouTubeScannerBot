...
def last_backup(args):
    backup_files = [p for p in args.dir.glob("*.sql") if PATTERN.match(p.name)]
...
def main():
    ...
    new_backup_path_parser = subparsers.add_parser(
        "new", help="generate path to new backup file with current datetime"
    )
    new_backup_path_parser.set_defaults(func=new_backup)

    # GET LAST BACKUP IN DIR
    last_backup_parser = subparsers.add_parser(
        "last", help="return last backup file in backups directory"
    )
    last_backup_parser.set_defaults(func=last_backup)
